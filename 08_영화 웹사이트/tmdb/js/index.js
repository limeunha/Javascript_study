...
const options = {
   ...
   headers: {
      ...
      // 보안을 위해서 서버에게 주는 인증키
      Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyZjdjNTkwZjE1Mjg2MzI4OGMyMWVlODVjNjVlM2I2NCIsIm5iZiI6MTcyOTgyNTc3Ni45NDc1NjksInN1YiI6IjY3MWFmMzJhNDU0MmUzNzFmZTBhNmQyMyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.bN9KP6Zy2spMDN7_21oDfHQOjIvn5yr-tIWMGWiwxmQ',
   },
}
...
사용자는 다른작업을 할수 X -> 따라서 request 작업은 비동기로 진행된다
*/
...
const getPlayingMovies = async (url) => {
   try {
      const response = await fetch(url, options) // 서버에서 데이터 가져올떄까지 기다린다
      //console.log(response) // response 정보 + 데이터 , 기다렸다가 다 가지고 오면 실행
...
//    .catch((err) => console.error(err)) // request할떄 문제 발생시 실행
